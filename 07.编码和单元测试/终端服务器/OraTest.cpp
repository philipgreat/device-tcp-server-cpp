...
int testHireDaoHire(OLFO  *ora1,OLFO  *ora2,OLFO  *ora3)
{
	...
	const char *args[]={
"delete from T_Terminal_Info","delete from T_SDR_200909",
"delete from T_Org_Info","insert into T_Org_Info(Org_ID,Superior_Org_ID)values('hotel','broker')",
"insert into T_Terminal_Info(Imei_Number,Uim_Number,msisdn,status,room_no,hotel_id)values('012345678901234','ABCDEFGHIJKLMNO','13902392590',1010000,'room','hotel')",
	NULL};
	executeUpdate(ora3,args);
	...
	CHECK_STRING("200909",CommonDao::getString(ora1,tempbuffer,"select to_char(Current_Hire_Time,'YYYYMM') from T_Terminal_Info"));
	CHECK_NUMBER(1,CommonDao::getInt(ora1,"select count(*) from T_SDR_200909"));
...
"insert into T_Terminal_Info(Imei_Number,Uim_Number,msisdn,status,room_no,hotel_id)values('012345678901234','ABCDEFGHIJKLMNO','13902392590',1010000,'room','hotel')",
	...
	CHECK_NUMBER(1,CommonDao::getInt(ora1,"select count(*) from T_Terminal_Info"));
	CHECK_STRING("200909",CommonDao::getString(ora1,tempbuffer,"select to_char(Current_Hire_Time,'YYYYMM') from T_Terminal_Info"));
	CHECK_NUMBER(0,CommonDao::getInt(ora1,"select status from T_SDR_200909"));
	...
	CHECK_NUMBER(1,CommonDao::getInt(ora1,"select status from T_SDR_200909"));
	...
	CHECK_STRING("200909",CommonDao::getString(ora1,tempbuffer,"select to_char(Current_Hire_Time,'YYYYMM') from T_Terminal_Info"));
	//应该有2个账单生成了
	CHECK_NUMBER(2,CommonDao::getInt(ora1,"select count(*) from T_SDR_200909"));
	...
	CHECK_NUMBER(12,CommonDao::getInt(ora1,"select count(*) from T_SDR_200909"));
	return 0;
}
...
"insert into T_Terminal_Info(Imei_Number,Uim_Number,msisdn,status,room_no,hotel_id)values('012345678901234','ABCDEFGHIJKLMNO','13902392590',1010000,'room','hotel')",
	...
	CHECK_NUMBER(1,CommonDao::getInt(ora1,"select status from T_SDR_200909"));
	CHECK_NUMBER(7,CommonDao::getInt(ora1," select (END_TIME-start_time)*86400 from T_SDR_200909"));
	//租用了7秒时间
#endif
...
select * from T_SDR_200909;

select * from t_access_log;
select * from T_SDR_200909;
select * from t_hire_restore_log;

select ID||'|'||IMEI_NUMBER||'|'||UIM_NUMBER||'|'|| MSISDN||'|'|| HOTEL_ID||'|'||ROOM_NO||'|'||BROKER_ID||'|'||START_TIME||'|'||HIRE_SRC||'|'||HIRE_CONFIRM_OPErator||'|'||HIRE_CONFIRM_TIME||'|'||HIRE_NOTES||'|'||END_TIME||'|'||QUIT_SRC||'|'||QUIT_NOTES||'|'||STATUS from T_SDR_200909;
