...
public class StatementOfMonthForPersonService extends FlexService{
	...
	public FlexReponse doTransaction(FlexRequest request){
		...
		try{
			...
			logger.debug("-----------------------------人员月报表管理任务 成功完成-------------------------------");
		}catch(Exception e){
			logger.error(e.getMessage());
			reponse = new FlexReponse(request,-1,e.getMessage());	
			logger.debug("-----------------------------人员月报表管理任务 执行失败-------------------------------");
		}
	...
	private FlexReponse initStatement(FlexRequest request) throws Exception{
		...
		String sql="select sum(Cash)/100 CASH,sum(Real_Pay_Cash)/100 REAL_PAY_CASH,to_char(Create_Time,'yyyy-mm-dd') CREATE_TIME,sum(Free_Cash)/100 FREE_CASH " +
		" from V_SERVICE_CASH where OPER_USER='"+request.user_id+"' and Status='1' " +
			" and to_char(Create_Time,'yyyy-mm-dd') >= '" +startTime + "' and to_char(Create_Time,'yyyy-mm-dd') < '"+ endTime +"'  group by to_char(Create_Time,'yyyy-mm-dd')";
		
		List<Map<String,Object>> list = builder.queryList(sql);
		
		if(builder.SQL_Error_Code != 0){			
			throw new Exception("查询人员月报表信息，获取人员月报表出错，原因："+builder.SQL_Error_Code+" "+builder.SQL_Error_Message);
		}
		if(list.size()>0){
			String sql2="select sum(Cash)/100 CASH,sum(Real_Pay_Cash)/100 REAL_PAY_CASH,to_char(Create_Time,'yyyy-mm') CREATE_TIME,sum(Free_Cash)/100 FREE_CASH  from V_SERVICE_CASH " +
			" where OPER_USER='"+request.user_id+"' and Status='1'  and to_char(Create_Time,'yyyy-mm-dd') >= '" +startTime + "' and to_char(Create_Time,'yyyy-mm-dd') < '"+ endTime +"'  group by to_char(Create_Time,'yyyy-mm')";
