...
public class StatementOfDayForAgentService extends FlexService{
	...
	public FlexReponse doTransaction(FlexRequest request){
		...
		try{
			...
			logger.debug("-----------------------------代销商日报表管理任务 成功完成-------------------------------");
		}catch(Exception e){
			logger.error(e.getMessage());
			reponse = new FlexReponse(request,-1,e.getMessage());	
			logger.debug("-----------------------------代销商日报表管理任务 执行失败-------------------------------");
		}
	...
	private FlexReponse initStatement(FlexRequest request) throws Exception{
		...
		" from V_SERVICE_CASH where Broker_ID='"+request.org_id+"' and Status='1' " +
			" and to_char(Create_Time,'yyyy-mm-dd')=to_char(sysdate,'yyyy-mm-dd') group by Hotel_ID";
		List<Map<String,Object>> list = builder.queryList(sql);
		
		if(builder.SQL_Error_Code != 0){		
			throw new Exception("查询代销商日报表信息，获取代销商日报表出错，原因："+builder.SQL_Error_Code+" "+builder.SQL_Error_Message);
		}
		
		if(list.size()>0){
			String sql2="select sum(Real_Pay_Cash)/100 REAL_PAY_CASH,sum(Free_Cash)/100 FREE_CASH  from V_SERVICE_CASH where Status='1' and Broker_ID='"
				
				+request.org_id+"' and to_char(Create_Time,'yyyy-mm-dd')=to_char(sysdate,'yyyy-mm-dd')";
			List<Map<String,Object>> list2 = builder.queryList(sql2);
		
			Iterator iter = list2.iterator();
			while(iter.hasNext()){
				Map map = (Map<String,Object>)iter.next();
				map.put("HOTEL_ID", "                                总计：");
				list.add(map);
			}
		...
		if(builder.SQL_Error_Code != 0){		
			throw new Exception("查询代销商日报表信息，获取代销商人员日报表出错，原因："+builder.SQL_Error_Code+" "+builder.SQL_Error_Message);
		}
	...
	private FlexReponse QueryStatement(FlexRequest request) throws Exception{
		...
		String sql="select Hotel_ID,sum(Real_Pay_Cash)/100 REAL_PAY_CASH,sum(Free_Cash)/100 FREE_CASH " +
		" from V_SERVICE_CASH where Broker_ID='"+request.org_id+"' and Status='1' and to_char(Create_Time,'yyyy-mm-dd')='" +time+"' group by Hotel_ID";
		...
		if(list.size()>0){
			String sql2="select sum(Real_Pay_Cash)/100 REAL_PAY_CASH,sum(Free_Cash)/100 FREE_CASH  from V_SERVICE_CASH where Status='1' and Broker_ID='"
				...
				+request.org_id+"' and to_char(Create_Time,'yyyy-mm-dd')='"+time+"'";
			List<Map<String,Object>> list2 = builder.queryList(sql2);
